...
import { MdAddLocationAlt, MdDashboard, MdFeedback } from "react-icons/md";
...
const Sidebar = () => {
  ...
  const topLinks = [
    { name: "Dashboard", path: `/landlord/${landlordId}`, icon: <MdDashboard /> },
    { name: "Add Listing", path: `/landlord/${landlordId}/add-listings`, icon: <MdAddLocationAlt /> },
    { name: "My Listings", path: `/landlord/${landlordId}/my-listings`, icon: <FaHouseChimney /> },
    { name: "Inbox", path: `/landlord/${landlordId}/inbox`, icon: <FaInbox /> },
    { name: "Pricing", path: `/landlord/${landlordId}/pricing`, icon: <IoPricetags /> },
  ];
    ...
    >
      <div className="rounded-xl border border-emerald-400/30 bg-gradient-to-br from-emerald-500/20 to-emerald-600/10 p-4 shadow-lg backdrop-blur">
        <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-gradient-to-br from-emerald-400 via-emerald-500 to-teal-600 text-lg font-bold text-white shadow-lg">
          {(landlord?.email || "O").charAt(0).toUpperCase()}
        </div>
        ...
        <h2 className="mt-1 truncate text-base font-bold text-white">
          {landlord?.username || "House owner"}
        </h2>
        <p className="truncate text-xs text-slate-400">{landlord?.email || ""}</p>
      </div>
  ...
  return (
    <>
      <header className="sticky top-0 z-50 border-b border-blue-100 bg-white/90 px-4 py-3 shadow-lg shadow-blue-100/20 backdrop-blur-xl lg:hidden">
        <div className="flex items-center justify-between gap-3">
          <Link to={`/landlord/${landlordId}`} className="flex min-w-0 items-center gap-3">
          ...
          <div className="absolute inset-y-0 left-0">
            <div className="flex h-full flex-col">
              <div className="flex items-center justify-between border-b border-emerald-400/30 bg-gradient-to-r from-[#0A4174] to-[#0A3563] px-4 py-3 text-white">
                <span className="text-sm font-bold">LankaNest Owner</span>
                <button
                  type="button"
                  onClick={() => setMobileOpen(false)}
